...
const MIGRATIONS = [
	...
	[
		'00010',
		`CREATE TABLE IF NOT EXISTS moodie.default_mood (
	...
	[
		'00011',
		`CREATE TABLE IF NOT EXISTS moodie.default_need (
    relationshiptype text,
    id text,
    name text,
    active boolean,
	...
	[
		'00012',
		`CREATE TYPE IF NOT EXISTS moodie.mood (
    id text,
    name text,
    active boolean,
    passive boolean
	...
	[
		'00013',
		`CREATE TYPE IF NOT EXISTS moodie.need (
    id text,
    name text,
    active boolean,
    passive boolean
	...
	[
		'00014',
		`ALTER TABLE moodie.user_relationship ADD (
...
const BLANK_NEED_SEED_VALUES = [['blank', 'nothing', 'Nothing', false, true]];
...
const SEEDS: [string, any[]][] = [
	[
		'INSERT INTO moodie.default_mood (relationshiptype, id, name, positive, negative)',
		MOOD_SEED_VALUES
	],
	[
		'INSERT INTO moodie.default_need (relationshiptype, id, name, active, passive)',
		NEED_SEED_VALUES
	]
];
...
export const runMigrations = async () => {
	...
	await CASSANDRA_CLIENT.execute(
		`CREATE TABLE IF NOT EXISTS moodie.migrations (key text PRIMARY KEY)`
	);
	for (const [key, migration] of MIGRATIONS) {
		const res = await CASSANDRA_CLIENT.execute(
			`SELECT key FROM moodie.migrations WHERE key = ?`,
			[key],
			{ prepare: true }
		);
		if (res.rows.length === 0) {
			...
			await CASSANDRA_CLIENT.execute(`INSERT INTO moodie.migrations (key) VALUES (?)`, [key], {
				prepare: true
			});
		}
	}
